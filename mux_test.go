...
type routeTest struct {
	...
	scheme         string            // the expected scheme of the match
...
func TestSchemes(t *testing.T) {
	tests := []routeTest{
		...
		{
			title:       "Https route build",
			route:       new(Route).Host("aaa.bbb.ccc").Schemes("https"),
...
func testRoute(t *testing.T, test routeTest) {
	...
	if shouldMatch {
		...
		if uri.Host != "" && uri.Path != "" {
			...
			if uri.String() != u.String() {
				t.Errorf("(%v) URL not equal: expected %v, got %v -- %v", test.title, uri, u, getRouteTemplate(route))
				return
			}
		}
